...
-   🟡 **localStorage 관리 도구** - 저장 공간 사용량 표시, 수동 정리 기능 **진행 중**
    -   **구현 방향**: 좌측 도구 상자에 아코디언 메뉴로 추가
    -   **UI 구성**: 
        - 아코디언 헤더: 전체 localStorage 사용량 표시 (예: 2.3MB)
        - 펼침 시: 카테고리별 용량 상세 표시 + 정리 버튼들
    -   **기능**: 히스토리 정리, 미사용 키 정리, 전체 초기화
    -   **구현 단계**: 1) 용량 계산 유틸리티 → 2) 아코디언 UI → 3) 정리 기능
    -   **🔧 발견된 문제들 및 해결 상태**:
        1. ✅ **히스토리 정리 오작동 해결**: localStorage와 메모리 상태 동시 업데이트로 해결
        2. **📄 프로젝트 파일 검증 과도함**: 전체삭제 후 백업 저장은 되지만 저장된 파일이 열리지 않음 (유효성 검사 기준이 너무 타이트)
           - **결정**: 나중에 처리 (우선순위 낮음, 다른 작업 우선 진행)
           - **해결 방향**: 백업 파일 복원 시 검증 완화 또는 백업 전용 경로 추가
        3. **미사용 키 정리 테스트 어려움**: 미사용 키 상황 재현이 어려워 실제 동작 확인 필요
