...
- [x] **노드 복사/붙여넣기 고도화** ✅ **완료 (2025-01-18)** - 다중 노드 복사 시 내부 연결 관계 유지
  - **해결**: 다중 노드 복사 시 내부 연결 관계가 완벽하게 보존되도록 구현
  - **핵심 개선사항**:
    - **클립보드 확장**: `{ nodes: EditorNodeWrapper[], connections: ConnectionInfo[] }` 구조로 연결 정보 포함
    - **연결 분석**: `_extractInternalConnections()` - 선택된 노드들 간의 내부 연결 자동 추출
    - **상대적 위치 보존**: 복사된 노드들의 공간적 배치 관계 유지
    - **ID 매핑 시스템**: 원본 노드 ID → 새 노드 ID 매핑으로 연결 복원
    - **그룹화 업데이트**: 동일 노드의 여러 연결을 한 번에 처리하여 덮어쓰기 방지
  - **기술적 세부사항**:
    - `ConnectionInfo` 타입: 텍스트/선택지 연결 정보 구조화
    - `_setupPastedNodeLocalizationWithoutConnectionCleanup()`: 연결 보존하며 로컬라이제이션 설정
    - 노드별 업데이트 그룹화로 다중 선택지 연결 동시 복원
  - **품질**: 복잡한 선택지 분기 구조도 완벽하게 복사/붙여넣기 지원 🎉
...
#### 데이터 관리 및 기타 개선 🔧 **중요**
- [x] **노드 삭제 시 키 정리** - 삭제된 노드의 LocalizationStore 키 자동 제거 ✅ **완료 (2025-01-12)**
  - **문제**: 다중 노드 삭제 시 공유 키가 정리되지 않는 문제 (예: 2개 노드가 같은 키 사용 시)
  - **원인**: LocalizationStore 참조 타이밍 문제와 복잡한 사전 계산 로직
  - **해결**: "삭제 후 전체 스캔" 방식으로 완전 리팩터링
    - 기존: 삭제 전 상태 기준 복잡한 미리 계산 → 예측 오류 발생
    - 신규: 삭제 후 현재 실제 templateData 직접 스캔 → 100% 정확
  - **성능**: 100개 노드 제한 하에서 O(n) 스캔 비용 무시할 수준 
  - **품질**: 데이터 정합성 100% 보장, 코드 복잡성 대폭 단순화 (약 50줄 → 15줄)
  - **통합**: 단일/다중 삭제 동일한 로직 사용으로 일관성 확보
- [x] **키 편집 후 사용 개수 업데이트** - 키 수정 시 UI 갱신 문제 ✅ **리팩터링 중 확인 및 개선됨**
...
- [x] **IEditorStore 인터페이스 정리** - 불필요한 선언 및 구현부 제거 ✅ **완료 (2025-01-18)**
  - **문제 해결**: 도메인별 스토어와 중복되는 메서드 선언 및 상태 프로퍼티 중복 정리
  - **핵심 개선**:
...
- 세부 최적화
  ...
  - [ ] **타입 정의 정리**: 중복/미사용 타입 정리, 타입 파일 구조화
  - [ ] **상수 정의 중앙화**: 매직 넘버/문자열 constants 파일로 이동
  - [ ] **에러 메시지 코드화**: 콘솔 경고/에러 메시지 표준화
  - [ ] **import 구조 최적화**: 불필요한 의존성 정리, import 순서 표준화
  - [ ] **중복 코드 제거**: 반복되는 패턴 공통 함수로 추출
  - [ ] **번들 크기 최적화**: 불필요한 패키지 제거, 트리 셰이킹 최적화
  - [ ] **성능 최적화**: 불필요한 렌더링 방지, 메모이제이션 검토
