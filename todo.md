...
- [x] **F14 중복 키 배정 텍스트 입력 버그 수정** ✅ **완료 (2025-01-12)**
  - **문제**: 이미 키값이 배정된 스트링을 새 노드의 textarea에 입력 후 focus 해제 시 textarea가 비워지고 키 개수 표시 오류
  - **원인**: 중복 키 사용 시 updateNodeKeyReference만 호출하여 키 참조만 업데이트하고 실제 텍스트는 업데이트하지 않음
  - **해결**: 
    - PropertyPanel에서 중복 키 감지 시에도 updateNodeText/updateChoiceText를 호출하여 실제 텍스트 함께 업데이트
    - updateChoiceText에 키 생성 및 중복 키 처리 로직 추가
    - 실시간 키 사용 개수 계산 함수 구현으로 KeyDisplay 컴포넌트 실시간 업데이트 보장
  - **영향 범위**: 화자명, 대화 내용, 선택지 텍스트 모든 입력 필드
