...
###### 1.3.2-A: 결과 처리 헬퍼 통합 (1순위 - 최우선)
-   [ ] `_handle*LayoutResult` 3개 헬퍼 분석 및 공통 로직 추출
-   [ ] 단일 `_handleLayoutSystemResult` 통합 함수 생성
-   [ ] 매개변수 구조 설계 (layoutType, nodeCount 등)
-   [ ] 기존 3개 헬퍼를 통합 함수 호출로 대체
-   [ ] 기능 검증 및 49줄 → 15줄 감소 확인

###### 1.3.2-B: 레이아웃 실행 헬퍼 통합 (2순위 - 높은 효과)
-   [ ] `_run*LayoutSystem` 3개 헬퍼 분석 및 공통 로직 추출
-   [ ] 단일 `_runLayoutSystem` 통합 함수 생성
-   [ ] depth 매개변수 설계 (global, descendant, child)
-   [ ] 기존 3개 헬퍼를 통합 함수 호출로 대체
-   [ ] 기능 검증 및 112줄 → 45줄 감소 확인

###### 1.3.2-C: 키 수집 로직 통합 (3순위 - 중간 효과)
-   [ ] `_collect*KeysForCleanup` 2개 헬퍼 분석 및 공통 로직 추출
-   [ ] 단일 `_collectLocalizationKeys` 통합 함수 생성
-   [ ] 단일/다중 모드 매개변수 설계
-   [ ] 기존 2개 헬퍼를 통합 함수 호출로 대체
-   [ ] 기능 검증 및 64줄 → 25줄 감소 확인

###### 1.3.2-D: 노드 탐색 헬퍼 통합 (4순위 - 구조 개선)
-   [ ] `_find*Nodes` 2개 헬퍼 분석 및 공통 로직 추출
-   [ ] 단일 `_findRelatedNodes` 통합 함수 생성
-   [ ] depth 제한 매개변수 설계 (1=child, Infinity=descendant)
-   [ ] 기존 2개 헬퍼를 통합 함수 호출로 대체
-   [ ] 기능 검증 및 36줄 → 20줄 감소 확인

##### 1.3.3 미세 최적화 (잔여 개선사항)

###### 1.3.3-A: 상수 및 리터럴 공통화
-   [ ] 레이아웃 간격 상수들 통합 (HORIZONTAL_SPACING, VERTICAL_SPACING 등)
-   [ ] 노드 크기 관련 상수들 표준화
-   [ ] 매직 넘버를 명명된 상수로 교체

###### 1.3.3-B: 간단한 유틸리티 함수 추출
-   [ ] 반복되는 검증 로직 유틸리티화 (`validateNodeExists`, `isValidPosition`)
-   [ ] 공통 타입 가드 함수 추출
-   [ ] 간단한 계산 헬퍼 함수 표준화

##### 1.3.4 헬퍼 메서드 구조 최적화 (최종 정리)
-   [ ] 남은 헬퍼들의 명명 규칙 일관성 확보 (`_동사명사` 패턴)
-   [ ] 파라미터 구조 표준화 (옵션 객체 vs 개별 매개변수)
-   [ ] 불필요한 헬퍼 병합 및 최종 중복 제거
-   [ ] JSDoc 주석 일관성 확보

**성공 기준**:
-   **Phase 1.3.2**: 156줄 감소 (265줄 → 109줄, 59% 감소)
-   **Phase 1.3.3**: 추가 10-15줄 감소 
-   **Phase 1.3.4**: 구조 일관성 및 문서화 완료
-   **전체 목표**: Phase 1 총 92% 감소 (1,170줄 → 99줄)
-   **헬퍼 감소**: 45개 → 25개 (44% 감소)
-   **기능 100% 보존**: 모든 단계에서 철저한 검증
