...
##### 1.3.1 헬퍼 메서드 현황 분석 및 패턴 식별 (1-2일)
-   [ ] 생성된 모든 헬퍼 메서드들을 카테고리별로 분류
-   [ ] 유사한 구조/패턴을 가진 헬퍼들 그룹핑
-   [ ] 중복 코드 후보들 식별 및 우선순위 결정

##### 1.3.2 명확한 중복 코드 제거 (2-3일)
-   [ ] 가장 명백한 중복 코드부터 제거 (예: 동일한 검증 로직)
-   [ ] 리터럴 값이나 상수들의 공통화
-   [ ] 간단한 유틸리티 함수 추출

##### 1.3.3 공통 패턴 기반 유틸리티 추출 (3-4일)
-   [ ] 여러 헬퍼에서 반복되는 패턴을 독립 함수로 추출
-   [ ] 레이아웃 관련 공통 로직 통합 (예: `_buildNodeRelationMaps` 계열)
-   [ ] 검증 관련 공통 로직 통합 (예: `_validate***` 계열)

##### 1.3.4 헬퍼 메서드 구조 최적화 (2-3일)
-   [ ] 남은 헬퍼들의 명명 규칙 일관성 확보
-   [ ] 파라미터 구조 표준화
-   [ ] 불필요한 헬퍼 병합 및 최종 정리
